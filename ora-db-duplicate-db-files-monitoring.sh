#!/bin/bash
########################################################################################################
# Name          : ora-db-duplicate-db-files-monitoring.sh
# Author        : Sagar Fale
# Date          : 29/12/2022
#
# Description:  - This script will check if DB is up and running
#
# Usage         : ora-db-duplicate-db-files-monitoring.sh
#
...
tlog=`date "+ora_db_duplicate_dbfiles-log-%d%b%Y_%H%M".log`
...
sqlplus -s "/ as sysdba" <<EOF
   ...
   set pagesize 50000
   set markup html on
   spool duplicate_db_files_${value1}.html
   select a.tablespace_name,a.file_name,to_char(c.CREATION_TIME,'dd-mm-yy:hh24:mi:ss') "Creation Time" from dba_data_files a, v\$datafile c where 1< (select count(*) from dba_data_files b where substr(a.file_name,15,30)=substr (b.file_name,15,30))  and a.file_id=c.file# order by substr (a.file_name,15,30)  ;
   set markup html off
   exit
EOF
   
   if [ -s duplicate_db_files_${value1}.html ]; then
           tempvalue=`echo "Notify --> $HOSTNAME $ORACLE_SID Duplicate DB files"`
           a=${script_base}/duplicate_db_files_${value1}.html
           echo "${tempvalue}" >> ${logfile}
           sendemail_notify_t ${a};
   else
          echo "${value1} DB No Duplicate DB files found  .. " >> ${logfile}
   fi  
