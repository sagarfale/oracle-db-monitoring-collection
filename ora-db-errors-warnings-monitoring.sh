#!/bin/bash
########################################################################################################
# Name          : ora-db-errors-warnings-monitoring.sh
# Author        : Sagar Fale
# Date          : 29/12/2022
#
# Description:  - This script will check if DB is up and running
#
# Usage         : ora-db-errors-warnings-monitoring.sh - v2
#
...
tlog=`date "+ora_db_errors_warnings-log-%d%b%Y_%H%M".log`
...
#### Errors/Warnins/Messeges reported in 1hr
...
sqlplus -s "/ as sysdba" <<EOF
   ...
   set pagesize 50000
   set markup html on
   spool ora_alert_${value1}.html
   SELECT ORIGINATING_TIMESTAMP, decode(MESSAGE_LEVEL,1,'CRITICAL',2,'SEVERE',8,'IMP',16,'NORMAL'), MESSAGE_TEXT, PROBLEM_KEY FROM V\$DIAG_ALERT_EXT WHERE MESSAGE_TEXT LIKE '%ORA-%' AND ORIGINATING_TIMESTAMP > sysdate-1/24 ORDER BY ORIGINATING_TIMESTAMP DESC;
   set markup html off
   exit
EOF
   
   if [ -s ora_alert_${value1}.html  ]; then
           tempvalue=`echo "Notify --> $HOSTNAME $ORACLE_SID Alert logs error/warnings reported in 1hr"`
           echo "${tempvalue}" >> ${logfile}
           a=${script_base}/ora_alert_${value1}.html
           sendemail_notify_t ${a};
   else
          echo "${value1} DB No critical Errors/Warnings found .. " >> ${logfile}
   fi  
