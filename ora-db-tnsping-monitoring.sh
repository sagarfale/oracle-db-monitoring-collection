#!/bin/bash
########################################################################################################
# Name          : ora-db-tnsping-monitoring.sh
# Author        : Sagar Fale
# Date          : 29/12/2022
#
# Description:  - This script will check if DB is up and running
#
# Usage         : ora-db-tnsping-monitoring.sh
#
...
tlog=`date "+ora_dbcheck_tnsping-log-%d%b%Y_%H%M".log`
...
tnsping ${value1}
   if [ $? != 0 ] ; 
   then 
      tempvalue=`echo "Notify --> Hostname: ${HOSTNAME} DBANME: ${ORACLE_SID} --> tnsping failed"`
      echo "${tempvalue}" >> ${logfile}
      echo "${value1} DB tnsping failed " >> ${logfile}
      sendemail_notify ;
    else 
      tempvalue=`echo "Notify --> Hostname: ${HOSTNAME} DBANME: ${ORACLE_SID} --> tnsping Success"`
      echo "${tempvalue}" 
      echo "${tempvalue}" >> ${logfile}
   fi
...
### checking Database ###
   ...
   if [ "$output" = "READ WRITE" ] ; then 
       tempvalue=`echo "CenCal --> Hostname : $HOSTNAME DBNAME: $ORACLE_SID Up and Running"`
       #echo $tempvalue
       ...
       tempvalue=`echo "CenCal --> Hostname : $HOSTNAME DBNAME: $ORACLE_SID Down"`
